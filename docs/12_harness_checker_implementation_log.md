...
> 이 절의 인증 설정은 당시 기록이다. 현재 운영 경계는 `docs/23_서버_API_계약.md`의
> 2026-09-19 개정을 따른다. 공개 모드의 매직링크 대체 경로와 미구현 SMTP 모드는 허용되지 않는다.
...
## 2026-08-29 단계 진행 시각화 (v0.3.2)

"어디가 어디로 가는지 모르겠다" 피드백. 3단계 카드에 진행 상태를 입혔다: 완료=초록 테두리+✓ 배지, 현재=파란 테두리+파란 배지, 대기=흐림. 저장 위치 선택/폴백·대상 업로드·URL 입력·점검 시작 각 지점에서 갱신. 결과 카드의 판정 행도 결과 색(통과=초록, 검토·수정=주황, 점검 중=파란 테두리)으로 표시. headless Chrome 실렌더링으로 상태 전환 확인.

## 2026-08-29 tools 가로 3단계 재배치 (v0.3.3)

설치 안내를 세로 나열에서 **가로 3칸**(내려받기·적용하기·개발하기)으로 재배치 — 전체 흐름이 한 화면에 들어온다. 대형 다운로드 버튼은 히어로 우측으로 올리고(뱃지 동반), 버전·만료·경고는 1단계 칸에. 릴리스 피드 실패 시 히어로에 "내려받을 수 없습니다" + ZIP 접기 자동 열림 유지.

## 2026-08-29 결과 요약-보고서 불일치 수정 (v0.3.4)

사용자가 vibecode-checker 저장소를 실점검한 보고서(493건, warn 196, blocked)와 화면 요약이 어긋난다고 보고. 실측 대조 결과 **서버 기록과 보고서는 일치(493)** — 화면이 실측값 대신 파생 판정을 재가공한 것이 원인.

- ① "사람 검토 필요"가 건수(warn 196) 대신 판정명이 needs_review 일 때만 "필요"로 표시 → incomplete 판정에서 **0건으로 거짓 표시**. ② 의존성 검사가 불완전(unchecked)인데 "0건" 표시 → 거짓 안심. ③ blocked(수정 필요)를 incomplete 가 가려 "재점검 필요"로만 표시.
- **수정**: 서버 summary 에 체커 실측값(block_count·warn_count·blocked·highest_severity)을 그대로 전달하고, 화면은 그 숫자를 표시한다. 의존성 불완전은 "확인 불충분"으로, blocked+incomplete 는 "수정 필요 · 재점검"으로. 판정·검사 로직은 불변 — 표시 계층만 수정.
- 원칙 재확인: 화면 요약은 보고서와 같은 숫자를 말해야 한다. 파생 가공은 불일치의 씨앗.
- 부수: 메인 히어로 행동 버튼 확대(330px/23px) + 설명과의 여백 확대.

## 2026-08-29 검토요청 무반응·Manager 안내·간편/표준 구분 (v0.3.5)

실사용 3건 보고를 실측으로 확인해 수정.

- **검토요청 무반응**: 새로고침 뒤에는 화면이 완료된 점검을 기억하지 못해 버튼이 **조용히 아무것도 안 했다**(silent return). 이제 "내 점검 이력에서 요청하라"는 안내+링크를 보여주고, 성공 시에도 이력 링크를 함께 표시.
- **Manager 안내 과장**: 데모 설치 파일은 Manager 를 자동으로 띄우지 않고 **시작 메뉴**(Gyeonggi VibeCode Harness Demonstration 폴더)에 바로가기를 만든다(실기기 확인: `%LOCALAPPDATA%\Gyeonggi\VibeCodeHarness-Demo\manager.ps1`). 포털 문구를 사실대로 "시작 메뉴에서 실행"으로 수정, 버튼 계약도 갱신. **하네스 백로그**: ①설치 후 Manager 자동 실행(또는 설치 완료 화면) ②데모 설치 경로가 문서의 github-pilot 경로와 다름 — release-index 나 문서에 실경로 명시 필요.
- **간편/표준 리포트 구분 불가**: 차이는 실재(간편 dev-quick 493건 vs 표준 public-default-strict 515건 — medium 16·warn 218) 하나 **리포트·파일명 어디에도 방식 표기가 없어** 같아 보였다. 보고서 파일명을 `_간편점검`/`_표준점검` 으로 분리하고, 결과 카드에 "점검 방식" 행 추가, 모드 카드 설명을 실제 차이(고위험 위주 vs 전체 규칙)로 교체. 시나리오의 파일명 계약도 모드별로 강화.
