...
- 결과가 `고위험 없음`이어도 제출 가능 판정과 제출 패키지는 생성하지 않음
...
|---|---:|---:|---:|---:|
| `quick` | 고위험 코드만, 최대 700개 | 예, 직접 매니페스트 | 예 | 아니오 |
| `standard` | 전체 규칙, 최대 20,000개 | 예, 직접 매니페스트 | 예 | 선택 |
| `submission` | 예 | 예 | 예 | 예 |

`submission`은 검사 누락이 있으면 `submittable`이 될 수 없다.

`profile_fallback`, 최대 파일 수 초과, 의존성 검사 절단·판정 불가가 있으면 표준·제출점검은 `재점검 필요`로 표시한다.
