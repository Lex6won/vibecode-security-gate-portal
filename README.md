...
- 폴더·ZIP 업로드 또는 GitHub·정부 GitLab 주소로 점검 — 간편(개발 중) / 표준(제출 전)
...
최초 실행 전 `.env`에 `ADMIN_ID`, `ADMIN_INITIAL_PASSWORD`(12자 이상)를 설정하세요. 인증 정보는 해시로 저장되며 Git에 포함되지 않습니다. 로그인 후 관리자 화면에서 비밀번호를 변경할 수 있고, 변경 시 기존 세션은 폐기됩니다. 개발·운영에서 동일한 관리자 계정을 유지하려면 두 `.env`에 같은 두 값을 넣고 `ADMIN_CREDENTIALS_SYNC=true`를 설정한 뒤 각각 재시작합니다.
